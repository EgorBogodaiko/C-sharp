...
System.Console.Write("Введите количество глубин матрицы: ");
int depth = System.Convert.ToInt32(Console.ReadLine());
...
int[,] Arr3dTo2d(int[,,] arr)
{
    ...
    for (int j = 0; j < arr.GetLength(0); j++)
    {
        for (int k = 0; k < arr.GetLength(1); k++)
        {
            ...
            {
                to2d[i, 0] = arr[j, k, l]; System.Console.Write($"{arr[j, k, l]}");
                to2d[i, 1] = j; System.Console.Write($" {to2d[i, 1]}");
                to2d[i, 2] = k; System.Console.Write($" {to2d[i, 2]}");
                to2d[i, 3] = l; System.Console.Write($" {to2d[i, 3]}");
                to2d[i, 4] = j * 100 + 10 * k + l; System.Console.Write($" {to2d[i, 4]}");
                System.Console.WriteLine(" ");
                i++;
            }
        }
...
{
    System.Console.WriteLine(" ");
    System.Console.Write($"{arr[0, 0]} ({arr[0, 1]}, {arr[0, 2]}, {arr[0, 3]}) ");
    for (int i = 1; i < arr.GetLength(0); i++)
    {
        if (arr[i, 1] > arr[i - 1, 1])
        {
            System.Console.WriteLine(" ");
            System.Console.Write($"{arr[i, 0]} ({arr[i, 1]}, {arr[i, 2]}, {arr[i, 3]}) ");
        }
        else
            System.Console.Write($"{arr[i, 0]} ({arr[i, 1]}, {arr[i, 2]}, {arr[i, 3]}) ");
    }
...
void CheckForCapacity(int r, int c, int d, int minRnd, int maxRnd)
{
    ...
    if (gap < capacity)
    {
        System.Console.WriteLine("Для массива такого размера не хватит количества целых чисел из диапазона рандомных значений.");
        Environment.Exit(0);
    }
}